 ...
 * usage is extended to 2D to be implemented in image pattern
 * finding.
 *
 * Shule Yu
 * Jan 18 2018
...
std::pair<std::pair<int,int>,double> CrossStd2D(const std::vector<std::vector<T1>> &s1, const std::vector<std::vector<T2>> &s2){
    ...
    if (m1<=1 || m2<=1){
		std::cerr << "In " << __func__ << ": Input sequences length error ..." << std::endl;
        return {};
	}
    ...
    if (m1<m2 || n1<n2){
		std::cerr << "In " << __func__ << ": s2 size larger than s1 size ..." << std::endl;
        return {};
	}
