...
void STDCC(const std::vector<double> &s1, const std::vector<double> &s2, int &shift, double &STD){

	int xlen=s1.size(),ylen=s2.size();

    // If signals are too short, return an error flag.
    if (xlen<=1 || ylen<=1 || xlen<ylen){
		std::cerr << "In " << __func__ << ": X/Y Sequence length error !" << std::endl;
        return;
	}

    // Shifting and evaluating STD at every delay value.

	STD=std::numeric_limits<double>::max();
...
void STD2CC(const std::vector<std::vector<double>> &s1, const std::vector<std::vector<double>> &s2, int &mi, int &mj, double &STD){

	int xi=s1.size(),yi=s2.size(),xj=s1[0].size(),yj=s2[0].size();

    // If signals are too short, return an error flag.
    if (xi<=1 || xj<=1 || yi<=1 || yj<=1 || xi<yi || xj<yj){
		std::cerr << "In " << __func__ << ": X/Y Matrices size error !" << std::endl;
        return;
	}
