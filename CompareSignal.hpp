 ...
 * Return(s):
 *
 * pair<double,pair<double,double>> ans  ----  {V1,{V2,V3}} defined as above.
 *
 * Shule Yu
 * Jan 30 2018
 *
...
std::pair<double,std::pair<double,double>> CompareSignal(const std::vector<T1> &s1, const T2 &p1,
                                                         const std::vector<T3> &s2, const T4 &p2,
                                                         const T5 &delta, const T6 &t1, const T7 &t2,
                                                         const T8 &AmpLevel) {

	// check input.
    int P1=p1,P2=p2,n1=s1.size(),n2=s2.size();
	if (P1<0 || P1+1>=n1 || P2<0 || P2+1>=n2){
        std::cerr <<  __func__ << "; Error: peak position error ..." << std::endl;
		return {};
	}

	int W1=(int)ceil(t1/delta),W2=(int)ceil(t2/delta);
    if (W1>=W2) {
        std::cerr <<  __func__ << "; Error: Time window error ..." << std::endl;
		return {};
    }
	if(P1+W1<0 || P1+W1>=n1 || P1+W2<0 || P1+W2>=n1){
        std::cerr <<  __func__ << "; Error: Time window error on signal 1 ..." << std::endl;
		return {};
	}
	if(P2+W1<0 || P2+W1>=n2 || P2+W2<0 || P2+W2>=n2){
        std::cerr <<  __func__ << "; Error: Time window error on signal 2 ..." << std::endl;
		return {};
	}

	if (s1[P1]<0.95 || s2[P2]<0.95){
        std::cerr <<  __func__ << "; Error: s1 or s2 not pre-normalized ..." << std::endl;
		return {};
	}
    ...
    int shift;
    double ccc;
	CrossCorrelation(C1,C2,shift,ccc);
